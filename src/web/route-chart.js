...
function portraitHeading(text) {
  const p = document.createElement("p");
  p.className = "route-portrait-heading";
  p.textContent = text;
  return p;
}

function portraitSubheading(text) {
  const p = document.createElement("p");
  p.className = "route-portrait-subheading";
  p.textContent = text;
  return p;
}
...
export function renderRouteChart(container, data) {
  ...
  const portraitConnector = document.createElement("div");
  portraitConnector.className = "route-portrait-connector";
  portraitConnector.setAttribute("aria-hidden", "true");

  const forkWrap = document.createElement("div");
  forkWrap.className = "route-flow-fork";
  forkWrap.appendChild(forkSvgThreeVsFive());

  flow.appendChild(portraitConnector);
  ...
  clusterThree.appendChild(portraitHeading("三年段 · 高中／高職"));
  ...
  innerThree.appendChild(portraitSubheading("普高銜接示例 · 擇一"));
  ...
  clusterFive.appendChild(portraitHeading("五年段 · 五專 → 二技"));
